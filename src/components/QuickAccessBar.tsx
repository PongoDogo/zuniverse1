...
const QuickAccessBar = () => {
  ...
  const tabs = [
    { id: "continue" as Tab, label: "Continue", icon: Play, count: continueWatching.length },
    { id: "watchlist" as Tab, label: "Watchlist", icon: Heart, count: watchlist.length },
    { id: "pinned" as Tab, label: "Pinned", icon: Pin, count: pinned.length },
  ];
    ...
    switch (activeTab) {
      case "continue":
        return continueWatching.map(item => ({
          ...
          subtitle: item.mediaType === "tv" ? `S${item.season} E${item.episode}` : `${Math.round((item.duration - item.currentTime) / 60)}m left`,
      ...
      case "watchlist":
        return watchlist.map(item => ({
          ...
          subtitle: item.mediaType === "tv" ? "TV Show" : "Movie",
              ...
              <div className="max-h-[calc(60vh-50px)] overflow-y-auto p-2 space-y-1">
                {items.length === 0 ? (
                  <div className="py-8 text-center text-muted-foreground text-sm">
                    {activeTab === "continue" && "Nothing to continue"}
                    {activeTab === "watchlist" && "Watchlist is empty"}
                    {activeTab === "pinned" && "No pinned items"}
                  </div>
                ) : (
                  items.map((item) => (
                    <Link
                    ...
                    >
                      <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden flex-shrink-0 bg-muted">
                        <img
                          src={getImageUrl(item.image, "w200")}
                          alt={item.title}
